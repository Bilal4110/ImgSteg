...
class Main:  # Creates a class called "Main"
    ...
    def __init__(self):
        ...
        This function creates a new instance of the Main class.
            ...
            :Attribute self.delimiter: This string is used to separate and identify the encoding message from the image data during the decoding process.
    ...
    def open_image(self, img):
        ...
        cv2.waitKey(0)  # Keeps the image open indefinitely until a user decides to press a key, or click on the exit button. # Code taken from: https://pyimagesearch.com/2014/06/02/opencv-load-image/
    ...
    def binary_to_char(self, binaryData): # Code in this function has been taken from: https://www.javatpoint.com/image-steganography-using-python
        """
    ...
    def hide_text(self, image, hiddenMessage):
        ...
        It first calculates the sizes of both the image and the message (with delimiter) that needs to be encoded.
        This is to ensure that there enough available bytes to encode.
        If there is enough space, then the message is encoded within the least significant bits of the image's pixels.
        ...
        if binaryLength >= totalBytes: # Checks to see if the size of the binary message is greater or equal to the size of the message. This is to ensure that there is adequate space for encoding the message.
            ...
            for row in image: # This loop goes over each row present within the image. Essentially this a line of pixels. Loop taken from:  https://medium.com/towards-data-science/hiding-data-in-an-image-image-steganography-using-python-e491b68b1372
                for pixel in row: # This loop iterates over each pixel present in that specific row.  Loop taken from: https://medium.com/towards-data-science/hiding-data-in-an-image-image-steganography-using-python-e491b68b1372
                    r, g, b = self.data_to_binary(pixel) # splits pixel into r,g,b and then converts them to their binary equivalent. Concept taken from:  https://medium.com/towards-data-science/hiding-data-in-an-image-image-steganography-using-python-e491b68b1372
                    for x in range(3): # loops 0 - 2 (0 = Red, 1 = Green, and 2 = Blue).
                        if Index < binaryLength: # Checks to see if the index is less than the length of the binary message.
                            ...
                            pixel[x] = modifiedLSB + int(binaryMessage[Index])  # Adds each binary value from the message into the LSBs of the color channels (r,g,b) depending on the position of the index.
    ...
    def encode(self):
        ...
        Encodes a message into a PNG image file. The user is asked to input a .png image file as well as a message to encode.
         The hide_text function will be called to embed the message into the image.
        ...
        """
        print("===========================")
        fileName = input("\nImage Filename with Extension (PNG files only): ") # Ask the user to input an image file name. Due to compression concerns only PNG files are accepted by this program.
        if fileName.endswith("png"): # Checks to verify that the filename adds with the .png suffix.
            ...
            print("\nImage has been encoded successfully.") # Informs the user that the image has been successfully encoded.
            print(f"\nImage has been saved as {dest}.")  # This tells the user the save location or the name of the newly encoded image.
    ...
    def decode(self):
        """
        This function asks a user to input an image to encode.
        It then extracts a hidden text from an encoded Image's pixels using the LSB method.
        Once the text is extracted, the delimiter is removed from the text to form a message which can be returned to the user.
            ...
            :rtype: str
        """
        print("=========================") # This has been used for design purposes.
        imageName = input("\nImage to decode: ") # Asks the user to supply an image to decode.
        encodedImage = self.open_image(imageName) # Opens the previously supplied image using the created open_image() function.
        ...
        for row in encodedImage: # This loop goes over each row present within the encoded image. Loop taken from:  https://medium.com/towards-data-science/hiding-data-in-an-image-image-steganography-using-python-e491b68b1372
            for pixels in row: # This loop iterates over each pixel present in that specific row. Loop taken from: https://medium.com/towards-data-science/hiding-data-in-an-image-image-steganography-using-python-e491b68b1372
                r, g, b = self.data_to_binary(pixels) # splits pixel into r,g,b and then converts them to their binary equivalent.
                for x in range(3): # loops 0 - 2 (0 = Red, 1 = Green, and 2 = Blue).
                    channel = [r, g, b][x] # Selects one of the r,g,b color channels.
                    binaryMessage += (channel[-1])# Adds the LSB value of each channel (r,g,b) to the BinaryMessage variable.
                    if self.delimiter in self.binary_to_char(binaryMessage): # This checks to see if the delimiter is found within the converted binary message. Concept taken from: https://stackoverflow.com/questions/9797446/how-to-remove-certain-characters-from-a-variable-python
                        text = self.binary_to_char(binaryMessage) # This converts the extracted binary values (with the delimiter) into ASCII characters.
    ...
    def menu(self):
        ...
        while True:
            ...
            \n2. Decode an Image.
            \n3. Exit the program.
            \nSelect Option: """) # This input field creates the interface for the menu system.
            ...
            if choice == "2": # if the user inputs "2" into the terminal"
                print(f"\nYour decoded message is: {self.decode()}") # Displays out the decoded message to the user.
                print("=================================================================")
...
if __name__ == "__main__": # Ensures that the program is only executed if it is being directly being run as a main program, and not when imported as a module in another python script.
    ...
    obj.menu() # Calls the menu function from the "main" class to display the menu to the end user.


"""Possible Implementations

To further improve and expand on the functionality of this program, the following suggestions have been provided:
